<?php 
    ...
    class Students extends Controller {
        ...
        public function profile(){
            guard();
            $id = $_SESSION['id'];
            $userId = $this->studentModel->getUserById($id);
            $applicantInfo = $this->studentModel->findUserInApplicantById($id);
            $active = "Active";
            $closed = "Closed";
            $semester = $this->studentModel->setActiveStatus($active);
            $activeSemester = $this->studentModel->getActiveStatus($active);
            // $course = $this->studentModel->findUserOnApplicant($users);
            $programs = $this->studentModel->collectProgram();
            $yearlevels = $this->studentModel->collectyearlevels();
            $subjects = $this->studentModel->collectSubjects();
            $status = $this->studentModel->checkStatusUser($id);
            $finalsubjects = $this->studentModel->getSubjectFromSubject($id);
            $total = 0;
            $data = [
                'total' => $total,
        ...
        public function edit($id){
            ...
            $user = $this->studentModel->findUserById($id);
            $subjects = $this->studentModel->getSubjectFromSubject($id);
              ...
              if($_SERVER['REQUEST_METHOD'] == 'POST'){
                ...
                $data=[
                  ...
                  if(empty($data['firstnameError']) && empty($data['lastnameError']) && empty($data['emailError'])) {
                        ...
                        if($this->studentModel->updateUserById($data)){
        ...
        public function password($id){
            ...
            if(empty($data['password'])) {
                ...
                if($this->studentModel->authenticate($data['email'], $data['password'])) {
                    $data['passwordMatch'] = 'Password match enter new password.';
                    $match = $data['passwordMatch'];
                    
                }else{
                    $data['passwordError'] = 'Password did not match.';
                }
            ...
            if(empty($data['passwordError']) && empty($data['newPasswordError']) && empty($data['confirmpasswordError'])) {
                $auth = $match;
                if($auth) {
                    $newpass = $data['newPassword'];
                    $hashedpass = password_hash($newpass, PASSWORD_DEFAULT);
                    
                    if($this->studentModel->updatePassword($data, $hashedpass)) {
            ...
            $this->view('students/password', $data);
        }

        public function confirm($id){
            guard();
            $applicants = $this->studentModel->findApplicantById($id);
            ...
            if($data['status'] == "Exam/Interview"){
                $status = "For Submission";
                
                if($this->studentModel->updateStatusById($data,$status)){
            ...
            $this->view('students/confirm',$data);
        }

       public function editprof($id){
        guard();
        $applicantform = $this->studentModel->findApplicantById($id);
        $id = $applicantform->id;
        ...
        $this->view('students/editprof',$data);
       ...
       public function editinfo($id){
        guard();
        $allgender = $this->studentModel->getGender();
        $courses = $this->studentModel->getAllCourses();
        $civil_statuses = $this->studentModel->getCivilStatus();
        $birthorders = $this->studentModel->getBirthOrder();
        $incomes = $this->studentModel->getAllincome();
        $medHistorys = $this->studentModel->getAllmedHistory();
        $control_no = $this->studentModel->getMaxControlNo();
        $applicationtype = $this->studentModel->getApplicationType();
        $active = "Active";
        $semester = $this->studentModel->getSemesterActive($active);
        $applicantUpdate = $this->studentModel->getStudentById($id);
        ...
        $oneCourse = $data['applicantUpdate']->firstCourse;
        $twoCourse = $data['applicantUpdate']->secondCourse;
        $threeCourse = $data['applicantUpdate']->thirdCourse;
        ...
        $data = [
            ...
            'firstCourse' => $oneCourse,
            'secondCourse' => $twoCourse,
            'thirdCourse' => $threeCourse,
            'firstName' => trim($_POST['firstName']),
        ...
        if(!in_array($typeOne, $allowedOne)){
            $data['reportCardError'] = 'Please submit again your requirements.';
        }
        ...
        if(!in_array($typeTwo, $allowedTwo)){
            $data['goodMoralError'] = 'Please submit again your requirements.';
        }
        ...
        if(!in_array($typeThree, $allowedThree)){
            $data['oneByOneError'] = 'Please submit again your requirements.';
        }
        ...
        //     $data['honorDismissalError'] = 'Please submit again your requirements.';
        // }
        ...
        if(empty($data['applicationTypeError']) && empty($data['firstCourseError']) && empty($data['secondCourseError']) && empty($data['thirdCourseError']) && empty($data['firstNameError']) && empty($data['MiddleNameError']) && empty($data['LastNameError']) && empty($data['suffixError']) && empty($data['genderError']) && empty($data['civilStatusError']) && empty($data['AgeError']) && empty($data['dateOfBirthError']) && empty($data['ReligionError']) && empty($data['NationalityError']) && empty($data['MarriedError']) && empty($data['currentStreetNoError']) && empty($data['currentBarangayError']) && empty($data['currentMunicipalityError']) && empty($data['permanentStreetNoError']) && empty($data['permanentBarangayError']) && empty($data['permanentMunicipalityError']) && empty($data['fatherFullNameError']) && empty($data['fatherOccupationError']) && empty($data['fatherContactError']) && empty($data['motherFullNameError']) && empty($data['motherOccupationError']) && empty($data['motherContactError']) && empty($data['guardianFullNameError']) && empty($data['guardianOccupationError']) && empty($data['guardianContactError']) && empty($data['noSiblingError']) && empty($data['birthOrderError']) && empty($data['IncomeError']) && empty($data['elemSchoolError']) && empty($data['elemAddressError']) && empty($data['elemGraduateError']) && empty($data['secondarySchoolError']) && empty($data['secondaryAddressError']) && empty($data['secondaryGraduateError']) && empty($data['seniorHsSchoolError']) && empty($data['seniorHsAddressError']) && empty($data['seniorHsGraduateError']) && empty($data['alsSchoolError']) && empty($data['alsAddressError']) && empty($data['alsGraduateError']) && empty($data['collegeSchoolError']) && empty($data['collegeAddressError']) && empty($data['collegeGraduateError']) && empty($data['medicationError']) && empty($data['allergicToError']) && empty($data['specifyOtherError']) && empty($data['medicalHistoryError']) && empty($data['applicantContactError']) && empty($data['applicantEmailError']) && empty($data['reportCardError']) && empty($data['goodMoralError']) && empty($data['oneByOneError']) && empty($data['conditionError'])){
            // &&  empty($data['honorDismisalError']) 
                ...
                if( $this->studentModel->updateApplicant($users_id, $data, $newNameOne, $newNameTwo, $newNameThree, $control_no,$semester,$academic_year)){
                    // $newNameFour,
                        header('location:' . URLROOT . '/students/profile');
        ...
        $this->view('students/editinfo', $data);
    }
